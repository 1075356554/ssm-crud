            ...
            <div class="modal-body">
                <form class="form-horizontal">
                    <div class="form-group">
                        <input type="hidden" name="empId" id="emp_update_id">
...
<script type="text/javascript">
    ...
    var pageNum;
    // var size;//当前页实际数据条数
    //1.页面加载完成后，发送一个ajax请求，渠道分页数据并显示
    $(function () {
        //去首页
        to_page(1);
    });
    ...
    function build_page_info(result) {
        ...
        $("#page_info_area").append("当前第" + pageInfo.pageNum + "页,共" +
            pageInfo.pages + "页," + pageInfo.total + "条记录");
    ...
    //直接click无法绑定成功，因为编辑按钮是通过ajax生成的，绑定时按钮还不存在，解决方法：
    //1.可以在创建按钮时绑定事件
    //2.绑定点击 live() 即使是后加的元素也能触发事件，但live方法在新版jQuery中被删除了，使用on继续替代
    $(document).on("click",".edit_btn",function () {
        reset_form($("#empUpdateModal form"));
        //弹出模态框之前要查出部门信息和当前员工信息
        //1.显示部门信息
    ...
    function getEmp(id) {
        $.ajax({
           ...
           success:function (result) {
                ...
                $("#emp_update_id").val(empData.empId);
           }
        });
    }
